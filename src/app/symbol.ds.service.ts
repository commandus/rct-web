 ...
 */
export class SymbolDataSource implements DataSource<Symbol> {
  private subject = new BehaviorSubject<Symbol[]>([]);
  ...
  connect(collectionViewer: CollectionViewer): Observable<Symbol[]> {
  ...
  load(
    ...
    .subscribe(
      value => {
        this.count = value.symbol.length;
        this.subject.next(value.symbol);
